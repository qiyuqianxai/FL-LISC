from nets import metor_nets
from torch.utils.data import Dataset,DataLoader
import torchvision
import json
from nets.channel_nets import basic_DNN,DSN
...
chkpt_dir = 'nets/mae_visualize_vit_large_ganloss.pth'
model_mae = prepare_model('nets/mae_visualize_vit_large_ganloss.pth', 'mae_vit_large_patch16')
model_mae.eval()
print('Model loaded.')
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_auto, mode='min', factor=0.1, patience=50, verbose=True, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08)
mask_ratio = 0 # 遮挡比例
...
def test():
    ...
    DSN_model = DSN().to("cuda")
    channel_model.load_state_dict(torch.load(checkpoints_dir+"/14_checkpoint.pth", map_location=lambda storage, loc: storage))
    channel_model.eval()
...
def AiT_exert_recon_images(src_path, dst_path, snr, channel_type,dataloader):
    ...
    channel_model.load_state_dict(
        torch.load(checkpoints_dir + f"/AwD_{channel_type}_snr25_checkpoint.pth", map_location=lambda storage, loc: storage))
    channel_model.train()
    with torch.no_grad():
